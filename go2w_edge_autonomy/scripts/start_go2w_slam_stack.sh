...
# Start the minimal GO2W SLAM runtime needed by the local edge planner:
#   1. Unitree LiDAR driver
#   2. Unitree SLAM backend
#
# This intentionally does not start keyDemo, slam_keyboard_client, rviz2, or
# slam_llm_command_client. The edge executor starts slam_llm_command_client only
# when it needs to send a structured navigation command.
...
LOG_DIR="${GO2W_SLAM_LOG_DIR:-${PROJECT_ROOT}/logs/slam_stack}"
...
run_unitree_binary() {
  ...
  if pgrep -f "${binary}|./${name}" >/dev/null 2>&1; then
...
wait_for_process() {
  ...
  while (( SECONDS < deadline )); do
    if pgrep -f "${UNITREE_SLAM_DIR}/${name}|./${name}" >/dev/null 2>&1; then
...
echo "logs:"
echo "  ${LOG_DIR}/xt16_driver.log"
echo "  ${LOG_DIR}/unitree_slam.log"
