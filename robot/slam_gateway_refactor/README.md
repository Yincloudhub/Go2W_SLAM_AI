...
7. Explicit supervised release uses `semantic_mobility_v5`. Registered targets pass directly to Unitree `mode=0` navigation and its native obstacle avoidance. GO2W does not approximate the robot's rotational swept footprint from a fixed left/right clearance threshold.
8. `supervised_reposition` is an internal post-failure recovery action, not a general relative-motion interface or a navigation preflight gate. After native navigation reports failure or sustained no progress, the local LLM may select only deterministic forward/backward/left/right clearance candidates. `MissionDecisionEngine` verifies the choice, and the Gateway uses `SportClient::Move` with zero yaw rate, a `0.50 m` distance cap, a `0.10 m/s` speed cap, and direction-specific XT16 reserves. Every step stops before world-state refresh. The legacy `supervised_departure` action remains a forward-only compatibility alias.
...
11. The default `1 s` obstacle freshness rule remains strict outside supervised release. During the explicit `0.10 m/s` supervised release, a previously valid XT16 summary may remain usable for at most `2 s`; this absorbs a missed 5 Hz update without permanently invalidating the navigation lease. Longer delay still fails closed.
