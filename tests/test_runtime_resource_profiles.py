...
class RuntimeResourceProfileTests(unittest.TestCase):
    def test_unified_d435_sidecar_has_bounded_yolo_profiles(self):
        text = (REPO_ROOT / "scripts" / "go2w_d435_perception_sidecar.sh").read_text(encoding="utf-8")
        self.assertIn('PROFILE="${GO2W_D435_PROFILE:-${GO2W_DEEPYOLO_PROFILE:-resident}}"', text)
        self.assertIn("resident)", text)
        self.assertIn("balanced)", text)
        self.assertIn("diagnostic)", text)
        self.assertIn("CAPTURE_EVERY_N=5", text)
        self.assertIn("INFERENCE_INTERVAL_MS=333", text)
        self.assertIn("GO2W_DEEPYOLO_RENDER_OVERLAY=0", text)
        self.assertIn("GO2W_D435_DEPTH_EVERY_N", text)
    ...
    def test_runtime_resource_snapshot_is_read_only(self):
        ...
        self.assertIn('labels = ["xt16_driver", "unitree_slam", "xt16_geometry", "d435_capture_owner", "d435_summary_reducer", "operator_web"]', text)
