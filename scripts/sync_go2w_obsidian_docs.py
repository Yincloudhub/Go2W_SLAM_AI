...
- XT16 四向净空只作为几何摘要；当前运行时安全策略统一命名为 `safe_guard`，
  注册目标直接交给 Unitree `mode=0` 原生规划和绕障。
  GO2W 不用固定左/右净空近似转向包络；原生导航失败或持续无进展后才生成
  有界平移恢复候选。
...
def update_frontmatter_date(text: str, updated: str = "2026-06-15") -> str:
