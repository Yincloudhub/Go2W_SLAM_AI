...
但当前两个侧车都会独立打开同一台 D435，存在 USB 带宽、设备占用和时间戳不一致问题。比赛版必须在采集端合并，而不是只在 JSON 层合并。
...
- RGB、depth、frame sequence 和 sensor timestamp 来自同一帧集。
- 深度处理不等待 YOLO；YOLO 负载过高时只丢弃 YOLO 帧，不影响深度摘要。
- 主输出为 `artifacts/d435_perception_summary.json`。
- 迁移期继续原子生成两个旧文件，保持现有 UI/Gateway 兼容。
- 迁移完成后，`go2w_stereo_depth_sidecar.sh` 和 `go2w_deepyolo_sidecar.sh` 只保留兼容入口，最终由一个 `go2w_d435_perception_sidecar.sh` 管理。
