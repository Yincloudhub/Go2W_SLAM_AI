...
毫秒。普通模式继续使用 `1 s` 新鲜度硬门；仅在显式工程放行且限速
`0.10 m/s` 时，已验证来源的摘要最大允许年龄为 `2 s`。这用于吸收一次短暂漏帧，
额外容忍窗口内最大位移为 `0.10 m`；超过 `2 s`、时间戳缺失或来源失效仍立即停止。
...
`0.03 m` 平移和 `0.08 rad` yaw 只是识别“后端完全没动”的观测分辨率，不是
左右净空、转向包络或碰撞许可阈值。
