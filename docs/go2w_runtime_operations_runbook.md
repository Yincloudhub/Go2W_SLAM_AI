...
XT16 四向净空以名义机身边缘为零点，不包含已过滤的机身回波：
...
上表只是 XT16 几何摘要的方向分级，不直接等同于受监督导航许可。
当前运行时策略统一命名为 `safe_guard`：

1. 注册点目标直接下发 Unitree `mode=0`，由原生规划器根据机器狗外形和局部地图
   判断起步转向与绕障。
2. GO2W 不再用固定的左/右单点净空近似机身旋转扫掠区域，也不在导航前抢先挪位。
3. 只有原生导航持续无进展后，XT16 才根据当时的前、后、左、右直线净空生成
   不超过 `0.50 m` 的有界恢复候选；候选不声称转向可行。
4. 本地 LLM 只从候选中选择下一步；`MissionDecisionEngine` 复核候选和距离。
5. Gateway 将候选转换为一个有界的临时 Unitree `mode=0` 位姿目标，使用原生
   `0.20 m/s` 最低速度，并按前、后、侧方向分别保留 XT16 净空。转向和绕障仍由
   Unitree 局部规划器负责，不再并行启动 `SportClient` 底盘控制器。
6. 只允许一次恢复目标。成功后原目标直接重试 Unitree 原生导航，不再经过第二次
   LLM 或重复安全判定。
...
来源失效仍立即停止。内部有界恢复目标同样使用原生 `0.20 m/s` 最低速度。
...
原生导航持续无进展时，闭环先暂停并刷新世界状态，让 LLM 恢复策略层只从当前
前/后/左/右有界候选中选一步；`MissionDecisionEngine` 复核后执行一次零 yaw
平移，停止后把原目标交还 Unitree。不会连续盲走多步，也不会把
LLM 输出直接当底层速度。
