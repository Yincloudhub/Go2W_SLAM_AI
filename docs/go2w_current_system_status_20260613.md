...
作为静止机器人的全向运动否决。显式工程放行改由 `planner_mobility_v3` 判断
Unitree `mode=0` pose navigation 是否存在前向出发走廊；侧/后近物体变为
`0.1 m/s` 限速告警，前向 `<0.80 m`、定位/感知失效仍硬暂停。

2026-06-14 首次真实低速导航进一步确认：Unitree API 1102 已接受 `mode=0`
目标并开始运动，但初始规划可能先原地转向。若侧边贴近箱体，单纯“前方清晰”
不足以证明能完成起步转向。v3 因此增加转向包络门，并允许确定性执行器在前方
净空覆盖全程时先低速前移最多 `0.50 m`，暂停后再提交原拓扑目标。通用
`relative_motion` 仍不向 LLM 开放。

后续现场复验显示，即便目标方位与当前朝向近似对齐，Unitree 内部路径仍可能先
转向。因此 bounded departure 的触发不再只依赖方位差：目标仍较远且侧/后存在
保守告警时，也先建立前向缓冲区，再把控制交回厂商规划器。
